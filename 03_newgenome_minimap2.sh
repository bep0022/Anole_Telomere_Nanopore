...
#############################################################
#SBATCH --job-name=newgenome_Minimap2_Test1           # job name
...
#SBATCH --output=job-%j_newgenome_Minimap2_Test2.out          # Output file. %j is replaced with job ID
#SBATCH --error=job-%j_newgenome_Minimap2_Test2.err           # Error file. %j is replaced with job ID
...
REF=/hosted/biosc/SchwartzLab/ReferenceGenomes/Anolis/GCA_037176765.1_rAnoSag1.mat_genomic.fna
...
minimap2 -a -x map-ont ${REF} ${WD}/Porechopped/*.fastq.gz  > newgenome_minimap2.sam

samtools view -bS newgenome_minimap2.sam > newgenome_minimap2.bam
samtools sort newgenome_minimap2.bam -o newgenome_minimap2_sorted.bam
samtools flagstat newgenome_minimap2_sorted.bam > newgenome_minimap2.stats.txt
samtools depth newgenome_minimap2_sorted.bam |  awk '{sum+=$3} END { print "Average = ",sum/NR}' > newgenome_minimap2.depth.txt
samtools consensus -f fasta newgenome_minimap2_sorted.bam  -o ${WD}/ConsensusSequences/newgenome_minimap2_SAM_cons.fa
